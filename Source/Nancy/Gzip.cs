...
namespace CommService
{
    ...
    {
        private static GzipSettings _settings;

        public static void EnableGzipCompression(this IPipelines pipelines, GzipSettings settings)
        {
            _settings = settings;
        ...
        {
            EnableGzipCompression(pipelines, new GzipSettings());
        }
        ...
        {
            return (_settings.MimeTypes.Any(x =>
                x == response.ContentType || response.ContentType.StartsWith($"{x};")));
        }
